module vgatop (CLK, VGA_VS, VGA_HS, VGA_R, VGA_G, VGA_B, DIST, centerDIST, weight);

    input CLK;
    output VGA_VS, VGA_HS;
    output [3:0] VGA_R, VGA_G, VGA_B;
    input [15:0] DIST, centerDIST, weight;
    ...
    initial begin
        ...
        text[42] = 8'h3A; // ':'
        text[43] = 8'h20; // ' '
        // Placeholder for WEIGHT DYNAMIC VALUE
        text[44] = 8'h30; // Thousands place (dynamic)
        text[45] = 8'h30; // Hundreds place (dynamic)
        text[46] = 8'h30; // Tens place (dynamic)
        text[47] = 8'h30; // Units place (dynamic)
    end
    ...
    always @(posedge CLK) begin
        ...
        // Convert `Weight` to ASCII
        text[44] <= 8'h30 + (Weight / 1000) % 10;         // Thousands place
        text[45] <= 8'h30 + (Weight / 100) % 10;          // Hundreds place
        text[46] <= 8'h30 + (Weight / 10) % 10;           // Tens place
        text[47] <= 8'h30 + (Weight % 10);                // Units place
    end
    ...
    always @(*) begin
        ...
        if (xpos >= START_X && xpos < START_X + (15 * CHAR_WIDTH)) begin
            ...
            end else if (ypos >= START_Y + 3 * (CHAR_HEIGHT + LINE_SPACING) &&
                         ypos < START_Y + 4 * CHAR_HEIGHT + 3 * LINE_SPACING) begin
                // Row 4: "WEIGHT: {WEIGHT DYNAMIC VALUE}"
                line_start_y = START_Y + 3 * (CHAR_HEIGHT + LINE_SPACING);
                char_index = (xpos - START_X) / CHAR_WIDTH + 36;
                if (char_index < 48) char_code = text[char_index];
            end
